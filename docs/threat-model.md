...
| T17 | Native DLL is replaced with a hostile one | Loaded by absolute path from the ACL-protected install directory, never from `%PATH%` or the working directory, and never by bare name. The Authenticode signature is verified with `WinVerifyTrust` **before** the file is mapped, because loading a DLL executes its entry point and verifying afterwards would be verifying code that had already run. The ABI version is checked before any other function is called. A failure at any step leaves the library unloaded, which costs a support bundle and nothing else. **Implemented**, with an explicit `-allow-unsigned` for development that is off by default. |
